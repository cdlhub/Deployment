...
echo "---- [$(date)] BEGIN PiWind install ----"

echo "> cloning PiWind repository..."

cd /home/${OS_NAME}
git clone git://github.com/OasisLMF/OasisPiWind.git --branch <MODEL_RELEASE_TAG>
...
./load_data.py -s <SQL_IP> -n <SQL_ENV_NAME> -l <SQL_ENV_PASS> -a <KEYS_SERVICE_IP> -A <MODEL_KEYS_SERVICE_PORT> -o <OASIS_API_IP> -O <OASIS_API_PORT>
...
cd /home/${OS_NAME}
sed -i 's/__release_tag__/<MODEL_RELEASE_TAG>/g' oasispiwindkeysserver.yml
sed -i 's/__keys_service_port__/<MODEL_KEYS_SERVICE_PORT>/g' oasispiwindkeysserver.yml
